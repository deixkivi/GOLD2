...
if len(argv) == 2 and argv[1] == 'indi':
    ...
    def graph_data(conn):           
        # Rysowanie wykresu
        cur = conn.cursor()
        cur.execute("SELECT MID, ID, CODE, SREDNIA, EFFECTIVEDATE FROM GOLD WHERE CODE = :GBP",
        {"GBP": "GBP"})
        data = cur.fetchall()
        ...
        ax1.plot(effectivedate, srednia, '-', label="Średnia", color='r')
        """
        ...
        cur.execute("SELECT MID, ID, CODE, SREDNIA, EFFECTIVEDATE FROM GOLD WHERE CODE = :USD",
        {"USD": "USD"})
        data1 = cur.fetchall()
        ...
        cur.execute("SELECT MID, ID, CODE, SREDNIA, EFFECTIVEDATE FROM GOLD WHERE CODE = :EUR",
        {"EUR": "EUR"})
        data2 = cur.fetchall()
